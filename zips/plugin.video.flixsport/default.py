...
if _Edit.UseTMDB :
    ...
    if len(_Edit.TMDB_api) > 0 and len(setting('tmdb_key')) > 0:
        TMDB_api = setting('tmdb_key')
    koding.dolog(TMDB_api,line_info=True)
    if  len(TMDB_api) == 0:
...
def SKindex():
    ...
    try:
        ...
        try:
            if _Edit.BackUp != '':
                xbmc.executebuiltin("XBMC.Notification("+str(addon_name)+",We failed to reach a server. -  Trying BackUp Server,10000,"+str(addon_icon)+")")
                getData(_Edit.BackUp,'')
            else:
                raise
        except:
            BaseMode = koding.YesNo_Dialog(title=str(addon_name)+ ', Failed to reach server',message='[COLOR blue][I]Do You wish to continue in base mode?[/COLOR][/I]')
    if BaseMode != False:
        koding.dolog(TMDB_api,line_info=True)
        BYB.addDir('Search',str(TMDB_api),400,icon_Search,fanart_Search,'','','','')
        getSources()
        BYB.addDir('Tools and Settings','',36,icon_Setting ,fanart_Setting,'Tools and Settings','','','')
    else:
...
def getSources():
        if exists(addon_favorites):
            addDir('Favorites'          ,'url',4 ,icon_Favorite,fanart_Favorite,'','','','')
        if setting_true("browse_xml_database"):
            addDir('XML Database'       ,'http://xbmcplus.xb.funpic.de/www-data/filesystem/',15,addon_icon,addon_fanart,'','','','')
 
        if setting_true("browse_community"):
            addDir('Community Files'    ,'community_files',16,addon_icon,addon_fanart,'','','','')
 
        if exists(addon_history) and setting_true('searchhistorymenu'):
            addDir('Search History'     ,'history',25,icon_History,fanart_History,'','','','')
 
        if setting_true("searchyt"):
            addDir('Search:Youtube'     ,'youtube',25,icon_YouTube,fanart_YouTube,'','','','')
 
        if setting_true("searchDM"):
            addDir('Search:dailymotion' ,'dmotion',25,icon_DailyMotion,fanart_DailyMotion,'','','','')
 
        if setting_true("PulsarM"):
            addDir('Pulsar:IMDB'        ,'IMDBidplay',27,addon_icon,addon_fanart,'','','','')
        if setting_true('tmdb_use_own_data'):
            addDir('My TMDB Data',TMDB_api,300,icon_tmdb,fanart_tmdb,'','','','')
                ...
                if thumbnail == None:
                    if linkedUrl.startswith('#YTsearch'):
                        if _Edit.YouTubeIcon == '':
                            thumbnail = join(ART,'youtube.png')
                        else:
...
#hakamac code
    tmdbapi = False
    if Searchkey == False:
        KeyboardMessage = 'Name of TV show or movie'
        Searchkey = ''
    ...
    while FoundChannel != ReadChannel:
        ...
        if not BaseSearch.startswith('http')and not BaseSearch.startswith('#'):
            koding.dolog('Trying decode of link'+str(_Edit.MainBase),line_info=True)
            ...
            for a in all_items:
                ...
                if len(vurl) > 5 and TestName.find(Searchkey) > 0:
                    ...
                    if vurl.find('sublink') > 0:
                        ...
                        addLink(str(vurl),name,tmdb_icon,tmdb_fanart,tmdb_description,tmdb_genres,tmdb_date,True,None,'',1)
                    else: 
            ...
            try:
                if len(url) > 1:
                    ...
                    if isXMLSource:
                        ...
                        addDir(name.encode('utf-8'),ext_url[0].encode('utf-8'),1,icon_nextpage,fanart,'','','',None,'source')
                    elif isJsonrpc:
                    ...
                    koding.dolog('tmdbapi =  '+str(tmdbapi))
...
def urlsolver(url):
    if setting('Updatecommonresolvers') == 'true':
        l = join(home,'genesisresolvers.py')
        if xbmcvfs.exists(l):
            os.remove(l)
...
def search(site_name,search_term=None):
    thumbnail=''
    if os.path.exists(history) == False or setting('clearseachhistory')=='true':
        SaveToFile(history,'')
...
# if _Edit.UseTMDB :
#     TMDB_api    = decode(setting('tmdb_key')) if _Edit.TMDB_api == "" else decode(_Edit.TMDB_api)
#     if  len(TMDB_api) == 0  :
#         tmdbapi = koding.YesNo_Dialog(title='TMDB API Key',message='API key is missing do you wish to enter in settings?')
#         if tmdbapi          :            koding.Open_Settings(focus='2.1')
