...
class tvshows:
    ...
    def search(self):
        ...
        navigator.navigator().addDirectoryItem(32603, 'tvSearchnew', 'ThePapaw_cunt.png', 'DefaultTVShows.png')
        ...
        for (id,term) in dbcur.fetchall():
            if term not in str(lst):
                delete_option = True
                navigator.navigator().addDirectoryItem(term, 'tvSearchterm&name=%s' % term, 'ThePapaw_cunt.png', 'DefaultTVShows.png')
                lst += [(term)]
        dbcur.close()

        if delete_option:
            navigator.navigator().addDirectoryItem(32605, 'clearCacheSearch', 'ThePapaw_cunt.png', 'DefaultAddonProgram.png')
    ...
    def languages(self):
        ...
        for i in languages: self.list.append(
            {'name': str(i[0]), 'url': self.language_link % i[1], 'image': 'ThePapaw_cunt.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
    ...
    def userlists(self):
        ...
        self.list = userlists
        for i in range(0, len(self.list)): self.list[i].update({'image': 'ThePapaw_cunt.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
