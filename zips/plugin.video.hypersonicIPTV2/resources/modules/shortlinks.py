...
def Get():
	...
	auth = 'http://main.tvstreams.xyz:83/enigma2.php?username='+username+'&password='+password+'&type=get_live_categories'
	...
	if not auth=="":
		...
		xbmcgui.Dialog().ok('[COLOR darkgrey]tvpro[/COLOR]','[COLOR blue]M3U URL: [/COLOR]%s'%shortm3u,'','[COLOR blue]EPG URL: [/COLOR]%s'%shortepg)
	else:
		return
