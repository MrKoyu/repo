...
def get_params():
    koding.dolog("get_params")
    
    param_string = sys.argv[2]
    
    koding.dolog("get_params "+str(param_string))
    ...
    if param_string:
        ...
        for command in split_commands:
            koding.dolog("get_params command="+str(command))
            if len(command) > 0:
                if "=" in command:
    ...
    koding.dolog("get_params "+repr(commands))
    return commands

# Fetch text content from an URL
def read(url):
    koding.dolog("read "+url)
...
def read_body_and_headers(url, post=None, headers=[], follow_redirects=False, timeout=None):
    koding.dolog("read_body_and_headers "+url)

    if post is not None:
        koding.dolog("read_body_and_headers post="+post)
    ...
    koding.dolog("read_body_and_headers cookies_file="+ficherocookies)
    ...
    try:
        koding.dolog("read_body_and_headers importing cookielib")
        import cookielib
    except ImportError:
        koding.dolog("read_body_and_headers cookielib no disponible")
        # If importing cookielib fails
        # let's try ClientCookie
        try:
            koding.dolog("read_body_and_headers importing ClientCookie")
            import ClientCookie
        except ImportError:
            koding.dolog("read_body_and_headers ClientCookie not available")
            # ClientCookie isn't available either
            urlopen = urllib2.urlopen
            Request = urllib2.Request
        else:
            koding.dolog("read_body_and_headers ClientCookie available")
        ...
        koding.dolog("read_body_and_headers cookielib available")
    ...
    # one of the two cookie handling modules
        koding.dolog("read_body_and_headers Cookies enabled")

        if os.path.isfile(ficherocookies):
            koding.dolog("read_body_and_headers Reading cookie file")
            # if we have a cookie file already saved
            # then load the cookies into the Cookie Jar
            try:
                cj.load(ficherocookies)
            except:
                koding.dolog("read_body_and_headers Wrong cookie file, deleting...")
        ...
        if cookielib is not None:
            koding.dolog("read_body_and_headers opener using urllib2 (cookielib)")
            # if we use cookielib
            # then we get the HTTPCookieProcessor
            # and install the opener in urllib2
            if not follow_redirects:
                opener = urllib2.build_opener(urllib2.HTTPHandler(debuglevel=dolog_enabled),urllib2.HTTPCookieProcessor(cj),NoRedirectHandler())
            else:
                opener = urllib2.build_opener(urllib2.HTTPHandler(debuglevel=dolog_enabled),urllib2.HTTPCookieProcessor(cj))
            urllib2.install_opener(opener)

        else:
            koding.dolog("read_body_and_headers opener using ClientCookie")
    ...
    if post is None:
        koding.dolog("read_body_and_headers GET request")
    else:
        koding.dolog("read_body_and_headers POST request")
    
    # Añade las cabeceras
    koding.dolog("read_body_and_headers ---------------------------")
    for header in headers:
        koding.dolog("read_body_and_headers header %s=%s" % (str(header[0]),str(header[1])) )
    ...
    info = handle.info()
    koding.dolog("read_body_and_headers Response")

    returnheaders=[]
    koding.dolog("read_body_and_headers ---------------------------")
    for header in info:
        koding.dolog("read_body_and_headers "+header+"="+info[header])
    ...
    # Tiempo transcurrido
    fin = time.clock()
    koding.dolog("read_body_and_headers Downloaded in %d seconds " % (fin-inicio+1))
    koding.dolog("read_body_and_headers body="+data)
...
def find_multiple_matches(text,pattern):
    koding.dolog("find_multiple_matches pattern="+pattern)
...
def find_single_match(text,pattern):
    koding.dolog("find_single_match pattern="+pattern)
...
def add_item( action="" , title="" , plot="" , url="" , thumbnail="" , fanart="" , show="" , episode="" , extra="", page="", info_labels = None, isPlayable = False , folder=True ):
    koding.dolog("add_item action=["+action+"] title=["+title+"] url=["+url+"] thumbnail=["+thumbnail+"] fanart=["+fanart+"] show=["+show+"] episode=["+episode+"] extra=["+extra+"] page=["+page+"] isPlayable=["+str(isPlayable)+"] folder=["+str(folder)+"]")
...
def close_item_list():
    koding.dolog("close_item_list")

    xbmcplugin.endOfDirectory(handle=int(sys.argv[1]), succeeded=True)

def play_resolved_url(url):
    koding.dolog("play_resolved_url ["+url+"]")
...
def get_temp_path():
    koding.dolog("get_temp_path")

    dev = xbmc.translatePath( "special://temp/" )
    koding.dolog("get_temp_path ->'"+str(dev)+"'")

    return dev

def get_runtime_path():
    koding.dolog("get_runtime_path")

    dev = xbmc.translatePath( __settings__.getAddonInfo('Path') )
    koding.dolog("get_runtime_path ->'"+str(dev)+"'")

    return dev

def get_data_path():
    koding.dolog("get_data_path")
    ...
    koding.dolog("get_data_path ->'"+str(dev)+"'")

    return dev

def get_setting(name):
    koding.dolog("get_setting name='"+name+"'")

    dev = __settings__.getSetting( name )

    koding.dolog("get_setting ->'"+str(dev)+"'")

    return dev

def set_setting(name,value):
    koding.dolog("set_setting name='"+name+"','"+value+"'")

    __settings__.setSetting( name,value )

def open_settings_dialog():
    koding.dolog("open_settings_dialog")

    __settings__.openSettings()

def get_localized_string(code):
    koding.dolog("get_localized_string code="+str(code))
    ...
    koding.dolog("get_localized_string ->'"+dev+"'")

    return dev

def keyboard_input(default_text="", title="", hidden=False):
    koding.dolog("keyboard_input default_text='"+default_text+"'")
    ...
    koding.dolog("keyboard_input ->'"+tecleado+"'")

    return tecleado

def message(text1, text2="", text3=""):
    koding.dolog("message text1='"+text1+"', text2='"+text2+"', text3='"+text3+"'")
...
def message_yes_no(text1, text2="", text3=""):
    koding.dolog("message_yes_no text1='"+text1+"', text2='"+text2+"', text3='"+text3+"'")
...
def selector(option_list,title="Select one"):
    koding.dolog("selector title='"+title+"', options="+repr(option_list))
...
def set_view(view_mode, view_code=0):
    koding.dolog("set_view view_mode='"+view_mode+"', view_code="+str(view_code))

    # Set the content for extended library views if needed
    if view_mode==MOVIES:
        koding.dolog("set_view content is movies")
        xbmcplugin.setContent( int(sys.argv[1]) ,"movies" )
    elif view_mode==TV_SHOWS:
        koding.dolog("set_view content is tvshows")
        xbmcplugin.setContent( int(sys.argv[1]) ,"tvshows" )
    elif view_mode==SEASONS:
        koding.dolog("set_view content is seasons")
        xbmcplugin.setContent( int(sys.argv[1]) ,"seasons" )
    elif view_mode==EPISODES:
        koding.dolog("set_view content is episodes")
        xbmcplugin.setContent( int(sys.argv[1]) ,"episodes" )

    # Reads skin name
    skin_name = xbmc.getSkinDir()
    koding.dolog("set_view skin_name='"+skin_name+"'",line_info=True)

    try:
        if view_code==0:
            koding.dolog("set_view view mode is "+view_mode)
            view_codes = ALL_VIEW_CODES.get(view_mode)
            view_code = view_codes.get(skin_name)
            koding.dolog("set_view view code for "+view_mode+" in "+skin_name+" is "+str(view_code))
            xbmc.executebuiltin("Container.SetViewMode("+str(view_code)+")")
        else:
            koding.dolog("set_view view code forced to "+str(view_code))
            xbmc.executebuiltin("Container.SetViewMode("+str(view_code)+")")
    except:
        koding.dolog("Unable to find view code for view mode "+str(view_mode)+" and skin "+skin_name)
