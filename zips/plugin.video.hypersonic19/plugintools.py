...
# - Added selector
#---------------------------------------------------------------------------
import xbmc,xbmcplugin,xbmcaddon,xbmcgui,urllib.request,urllib.parse,urllib.error,urllib.request,re,sys,os,time,socket,gzip
...
def get_params(): # Parse XBMC params - based on script.module.parsedom addon    
    ...
    if param_string:
        ...
        for command in split_commands:
            ...
            if len(command) > 0:
                if "=" in command: split_command=command.split('='); key=split_command[0]; value=urllib.parse.unquote_plus(split_command[1]); commands[key]=value
                else: commands[command]=""
    _log("get_params "+repr(commands))
    return commands
# Fetch text content from an URL
def read(url): _log("read "+url); f=urllib.request.urlopen(url); data=f.read(); f.close(); return data
def read_body_and_headers(url,post=None,headers=[],follow_redirects=False,timeout=None):
    ...
    try: _log("read_body_and_headers importing cookielib"); import http.cookiejar # Let's see if cookielib is available
    except ImportError:
        ...
        except ImportError: _log("read_body_and_headers ClientCookie not available"); urlopen=urllib.request.urlopen; Request=urllib.request.Request # ClientCookie isn't available either
        ...
        _log("read_body_and_headers cookielib available"); urlopen=urllib.request.urlopen; Request=urllib.request.Request; cj=http.cookiejar.MozillaCookieJar() # importing cookielib worked
    ...
    if cj is not None: # we successfully imported # one of the two cookie handling modules
        ...
        if cookielib is not None:
            ...
            if not follow_redirects: opener=urllib.request.build_opener(urllib.request.HTTPHandler(debuglevel=http_debug_log_enabled),urllib.request.HTTPCookieProcessor(cj),NoRedirectHandler())
