...
class movies:
    ...
    def search(self):
        navigator.navigator().addDirectoryItem(32603, 'movieSearchnew', 'diamond_is_a_cunt.png', 'DefaultMovies.png')
        search_history = control.setting('moviesearch')
        if search_history:
            for term in search_history.split('\n'):
                if term:
                    navigator.navigator().addDirectoryItem(term, 'movieSearchterm&name=%s' % term, 'diamond_is_a_cunt.png', 'DefaultMovies.png')
            navigator.navigator().addDirectoryItem(32605, 'clearCacheSearch', 'diamond_is_a_cunt.png', 'DefaultAddonProgram.png')
    ...
    def languages(self):
        ...
        for i in languages: self.list.append({'name': str(i[0]), 'url': self.language_link % i[1], 'image': 'diamond_is_a_cunt.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def certifications(self):
        ...
        for i in certificates: self.list.append({'name': str(i), 'url': self.certification_link % str(i).replace('-', '_').lower(), 'image': 'diamond_is_a_cunt.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def years(self):
        ...
        for i in range(int(year)-0, 1900, -1): self.list.append({'name': str(i), 'url': self.year_link % (str(i), str(i)), 'image': 'diamond_is_a_cunt.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def movieDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                cm.append(('MTB Settings', 'RunPlugin(%s?action=openSettings&query=(0,0))' % sysaddon))
