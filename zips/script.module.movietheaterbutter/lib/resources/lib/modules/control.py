...
def addonIcon():
    ...
    if not (art is None and theme in ['-', '']):
        return os.path.join(art, 'fuckoff.png')
...
def addonThumb():
    ...
    art = artPath()
    if not (art is None and theme in ['-', '']):
        return os.path.join(art, 'diamond_is_a_cunt.png')
    elif theme == '-':
...
def addonFanart():
    ...
    art = artPath()
    if not (art is None and theme in ['-', '']):
        return os.path.join(art, 'youaskedforit.jpg')
...
def addonNext():
    ...
    art = artPath()
    if not (art is None and theme in ['-', '']):
        return os.path.join(art, 'next2.png')
    return 'DefaultVideo.png'
...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.movietheaterbutter.artwork)'):
        return os.path.join(
            xbmcaddon.Addon('script.movietheaterbutter.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.movietheaterbutter.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.movietheaterbutter.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.movietheaterbutter.metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.movietheaterbutter.metadata').getAddonInfo('path'),
