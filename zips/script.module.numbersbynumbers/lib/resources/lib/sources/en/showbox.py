...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            if any(s in token for s in ("oload","openload")):
                valid, host = source_utils.is_host_valid(token, hostDict)
                sources.append({'source':host,'quality':'720p','language': 'en','url':token,'info':[],'direct':False,'debridonly':False})
            else:
                ...
                for k, v in manifest.iteritems():
                    try:
                        quality = k + 'p' if int(k) >= 720 else 'SD'
                        sources.append({
                            'source': 'CDN',
                            'quality': quality,
                            'language': 'en',
