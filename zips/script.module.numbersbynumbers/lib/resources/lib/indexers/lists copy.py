...
import base64,hashlib,os,random,re,requests,shutil,string,sys,urllib,urllib2,json,resolveurl,ssl,zipfile,urlparse, datetime,xbmc,xbmcgui
...
class indexer:
    ...
    def root_greyhat(self):#
        try:
            regex.clear()
            url = 'http://the-crew.xyz/xmlhat/greyhat/greyhat_main.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_247movies(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_247/247_movies.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_247shows(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_247/247_shows.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_247channels(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_247/247_channels.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_learningtv(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_learning/learningtv.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_eimportalmovies(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_portal/eim_movies.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_eimportalshows(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_portal/eim_shows.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_accountsrd(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_accounts/authorize.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_speedtest(self):#
        try:
            regex.clear()
            url = 'http://cellardoortv.com/numbers/num_speedtest/speedtest.xml'
            self.list = self.the_crew_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def youtube(self, url, action):
        try:
            ...
            if 'Tuner' in action:
                ...
                for i in self.list: i.update({'name': i['title'], 'poster': i['image'], 'nextaction': action, 'action': 'plugin', 'folder': False})
                self.addDirectory(self.list)
    ...
    def addSearch(self, url):
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.the_crew_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['The Crew']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
