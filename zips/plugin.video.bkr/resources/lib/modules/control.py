...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.bkr.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.bkr.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.bkr.artwork)') else setting('appearance.alt')
    if appearance == "bkr": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.bkr.artwork)')
